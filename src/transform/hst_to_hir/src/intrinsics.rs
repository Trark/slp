...
const INTRINSICS: &'static [IntrinsicDefinition] =
    &[(T_VOID_TY,
       "AllMemoryBarrier",
       &[],
       I0(Intrinsic::AllMemoryBarrier)),
      (T_VOID_TY,
       "AllMemoryBarrierWithGroupSync",
       &[],
       I0(Intrinsic::AllMemoryBarrierWithGroupSync)),
      (T_VOID_TY,
       "DeviceMemoryBarrier",
       &[],
       I0(Intrinsic::DeviceMemoryBarrier)),
      (T_VOID_TY,
       "DeviceMemoryBarrierWithGroupSync",
       &[],
       I0(Intrinsic::DeviceMemoryBarrierWithGroupSync)),
      (T_VOID_TY,
       "GroupMemoryBarrier",
       &[],
       I0(Intrinsic::GroupMemoryBarrier)),
      (T_VOID_TY,
       "GroupMemoryBarrierWithGroupSync",
       &[],
       I0(Intrinsic::GroupMemoryBarrierWithGroupSync)),
      ...
      (T_FLOAT2_TY,
       "asfloat",
       &[T_FLOAT2],
       I1(Intrinsic::AsFloatF2)),
      (T_FLOAT3_TY,
       "asfloat",
       &[T_FLOAT3],
       I1(Intrinsic::AsFloatF3)),
      (T_FLOAT4_TY,
       "asfloat",
       &[T_FLOAT4],
       I1(Intrinsic::AsFloatF4)),

      (T_DOUBLE_TY,
       "asdouble",
       &[T_UINT, T_UINT],
       I2(Intrinsic::AsDouble)),
      ...
      (T_INT2_TY,
       "clamp",
       &[T_INT2, T_INT2, T_INT2],
       I3(Intrinsic::ClampI2)),
      (T_INT3_TY,
       "clamp",
       &[T_INT3, T_INT3, T_INT3],
       I3(Intrinsic::ClampI3)),
      (T_INT4_TY,
       "clamp",
       &[T_INT4, T_INT4, T_INT4],
       I3(Intrinsic::ClampI4)),
      (T_FLOAT_TY,
       "clamp",
      ...
      (T_FLOAT2_TY,
       "clamp",
       &[T_FLOAT2, T_FLOAT2, T_FLOAT2],
       I3(Intrinsic::ClampF2)),
      (T_FLOAT3_TY,
       "clamp",
       &[T_FLOAT3, T_FLOAT3, T_FLOAT3],
       I3(Intrinsic::ClampF3)),
      (T_FLOAT4_TY,
       "clamp",
       &[T_FLOAT4, T_FLOAT4, T_FLOAT4],
       I3(Intrinsic::ClampF4)),
      ...
      (T_FLOAT_TY,
       "distance",
       &[T_FLOAT1, T_FLOAT1],
       I2(Intrinsic::Distance1)),
      (T_FLOAT_TY,
       "distance",
       &[T_FLOAT2, T_FLOAT2],
       I2(Intrinsic::Distance2)),
      (T_FLOAT_TY,
       "distance",
       &[T_FLOAT3, T_FLOAT3],
       I2(Intrinsic::Distance3)),
      (T_FLOAT_TY,
       "distance",
       &[T_FLOAT4, T_FLOAT4],
       I2(Intrinsic::Distance4)),
...
pub fn get_method(object: &ObjectType, name: &str) -> Result<MethodDefinition, ()> {
    ...
    fn find_method_0(object: &ObjectType,
                     defs: &'static [MethodT0],
                     name: &str)
    ...
    type MethodT1 = (&'static str,
                     Box<Fn(&Type) -> (Type, Vec<ParamType>)>,
                     IntrinsicFactory);
    fn find_method_1(object: &ObjectType,
                     t0: &Type,
                     defs: &[MethodT1],
                     name: &str)
                     -> Result<MethodDefinition, ()> {
    ...
    match *object {
        ObjectType::Buffer(ref data_type) => {
            let methods: &[MethodT1] = &[("Load",
                                          Box::new(|ty: &Type| (ty.clone(), vec![T_INT])),
                                          I2(Intrinsic::BufferLoad))];
            find_method_1(object, &Type::from_data(data_type.clone()), &methods, name)
        }
        ObjectType::RWBuffer(ref data_type) => {
            let methods: &[MethodT1] = &[("Load",
                                          Box::new(|ty: &Type| (ty.clone(), vec![T_INT])),
                                          I2(Intrinsic::RWBufferLoad))];
            find_method_1(object, &Type::from_data(data_type.clone()), &methods, name)
        }
        ObjectType::StructuredBuffer(ref structured_type) => {
            let methods: &[MethodT1] = &[("Load",
                                          Box::new(|ty: &Type| (ty.clone(), vec![T_INT])),
                                          I2(Intrinsic::StructuredBufferLoad))];
            find_method_1(object,
                          &Type::from_structured(structured_type.clone()),
                          &methods,
                          name)
        }
        ObjectType::RWStructuredBuffer(ref structured_type) => {
            let methods: &[MethodT1] = &[("Load",
                                          Box::new(|ty: &Type| (ty.clone(), vec![T_INT])),
                                          I2(Intrinsic::RWStructuredBufferLoad))];
            find_method_1(object,
                          &Type::from_structured(structured_type.clone()),
                          &methods,
                          name)
        ...
        ObjectType::RWTexture2D(ref data_type) => {
            let methods: &[MethodT1] = &[("Load",
                                          Box::new(|ty: &Type| (ty.clone(), vec![T_INT2])),
                                          I2(Intrinsic::RWTexture2DLoad))];
            find_method_1(object, &Type::from_data(data_type.clone()), &methods, name)
        }
        ObjectType::ByteAddressBuffer => {
            const METHODS: &'static [MethodT0] = &[("Load",
                                                    T_UINT_TY,
                                                    &[T_UINT],
                                                    I2(Intrinsic::ByteAddressBufferLoad)),
                                                   ("Load2",
                                                    T_UINT2_TY,
                                                    &[T_UINT],
                                                    I2(Intrinsic::ByteAddressBufferLoad2)),
                                                   ("Load3",
                                                    T_UINT3_TY,
                                                    &[T_UINT],
                                                    I2(Intrinsic::ByteAddressBufferLoad3)),
                                                   ("Load4",
                                                    T_UINT4_TY,
                                                    &[T_UINT],
                                                    I2(Intrinsic::ByteAddressBufferLoad4))];
        ...
        ObjectType::RWByteAddressBuffer => {
            const METHODS: &'static [MethodT0] = &[("Load",
                                                    T_UINT_TY,
                                                    &[T_UINT],
                                                    I2(Intrinsic::RWByteAddressBufferLoad)),
                                                   ("Load2",
                                                    T_UINT2_TY,
                                                    &[T_UINT],
                                                    I2(Intrinsic::RWByteAddressBufferLoad2)),
                                                   ("Load3",
                                                    T_UINT3_TY,
                                                    &[T_UINT],
                                                    I2(Intrinsic::RWByteAddressBufferLoad3)),
                                                   ("Load4",
                                                    T_UINT4_TY,
                                                    &[T_UINT],
                                                    I2(Intrinsic::RWByteAddressBufferLoad4)),
                                                   ("Store",
                                                    T_VOID_TY,
                                                    &[T_UINT, T_UINT],
                                                    I3(Intrinsic::RWByteAddressBufferStore)),
                                                   ("Store2",
                                                    T_VOID_TY,
                                                    &[T_UINT, T_UINT2],
                                                    I3(Intrinsic::RWByteAddressBufferStore2)),
                                                   ("Store3",
                                                    T_VOID_TY,
                                                    &[T_UINT, T_UINT3],
                                                    I3(Intrinsic::RWByteAddressBufferStore3)),
                                                   ("Store4",
                                                    T_VOID_TY,
                                                    &[T_UINT, T_UINT4],
                                                    I3(Intrinsic::RWByteAddressBufferStore4))];
            find_method_0(object, METHODS, name)
        }
        _ => Err(()),
    }
