...
impl ConstructorSlot {
    pub fn direct_to_hir(&self) -> Result<hir::ConstructorSlot, DirectToHirError> {
        Ok(hir::ConstructorSlot {
            arity: self.arity,
            expr: self.expr.direct_to_hir_recur(RequiredVt::Rvt)?,
        })
    }
}
    ...
    fn direct_to_hir_recur(&self, vt: RequiredVt) -> Result<hir::Expression, DirectToHirError> {
        match *self {
            ...
            Expression::TernaryConditional(ref cond, ref lhs, ref rhs) => {
                let cond_hir = Box::new(cond.direct_to_hir_recur(RequiredVt::Rvt)?);
                let lhs_hir = Box::new(lhs.direct_to_hir_recur(RequiredVt::Rvt)?);
                let rhs_hir = Box::new(rhs.direct_to_hir_recur(RequiredVt::Rvt)?);
                Ok(hir::Expression::TernaryConditional(
                    cond_hir, lhs_hir, rhs_hir,
                ))
            }
            Expression::Swizzle(ref composite, ref swizzle) => {
                if vt == RequiredVt::Rvt {
                    let composite_hir = Box::new(composite.direct_to_hir_recur(vt)?);
            ...
            Expression::ArraySubscript(ref arr, ref index) => {
                let arr_hir = Box::new(arr.direct_to_hir_recur(vt)?);
                let index_hir = Box::new(index.direct_to_hir_recur(RequiredVt::Rvt)?);
                Ok(hir::Expression::ArraySubscript(arr_hir, index_hir))
            }
            Expression::Texture2DIndex(_, _, _) | Expression::RWTexture2DIndex(_, _, _) => {
            ...
            Expression::Call(ref id, ref args) => {
                ...
                for arg in args {
                    let arg_hir = arg.direct_to_hir_recur(RequiredVt::Lvt)?;
            ...
            Expression::Cast(ref ty, ref expr) => {
                if vt == RequiredVt::Rvt {
                    let expr_hir = Box::new(expr.direct_to_hir_recur(vt)?);
            ...
            Expression::Intrinsic1(ref i, ref e1) => {
                ...
                let e1_hir = Box::new(e1.direct_to_hir_recur(p1_vt)?);
            ...
            Expression::Intrinsic2(ref i, ref e1, ref e2) => {
                ...
                let e2_hir = Box::new(e2.direct_to_hir_recur(p2_vt)?);
            ...
            Expression::Intrinsic3(ref i, ref e1, ref e2, ref e3) => {
                ...
                let e3_hir = Box::new(e3.direct_to_hir_recur(p3_vt)?);
                Ok(hir::Expression::Intrinsic3(
                    i.clone(),
                    e1_hir,
                    e2_hir,
                    e3_hir,
