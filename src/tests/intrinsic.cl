...
void test_texture_2d(image2d_t g_rwTexture2DFloat, image2d_t g_rwTexture2DInt, image2d_t g_rwTexture2DUInt, uint3 dtid)
{
	int2 coord;
	coord.x = (int)dtid.x;
	coord.y = (int)dtid.y;
	float4 read_load_f = read_imagef(g_rwTexture2DFloat, coord);
	int4 read_load_i = read_imagei(g_rwTexture2DInt, coord);
	uint4 read_load_ui = read_imageui(g_rwTexture2DUInt, coord);
}

__attribute__((reqd_work_group_size(8, 8, 1)))
kernel void MyKernel(__global uint4* g_roBuffer, __global struct testStruct* g_roStructuredBuffer, __global uint4* g_rwBuffer, __global struct testStruct* g_rwStructuredBuffer, image2d_t g_rwTexture2DFloat, image2d_t g_rwTexture2DInt, image2d_t g_rwTexture2DUInt)
{
	...
	test_texture_2d(g_rwTexture2DFloat, g_rwTexture2DInt, g_rwTexture2DUInt, dtid);
