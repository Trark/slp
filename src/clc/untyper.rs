...
impl Context {
    ...
    fn get_global_name(&self, id: &src::GlobalId) -> Result<dst::Identifier, UntyperError> {
        Ok(self.global_name_map.get(&GlobalType::Variable(id.clone())).expect("untyper: no global").clone())
    }

    fn get_function_name(&self, id: &src::FunctionId) -> Result<dst::Identifier, UntyperError> {
        Ok(self.global_name_map.get(&GlobalType::Function(id.clone())).expect("untyper: no function").clone())
    }

    fn get_struct_name(&self, id: &src::StructId) -> Result<dst::Identifier, UntyperError> {
        Ok(self.global_name_map.get(&GlobalType::Struct(id.clone())).expect("untyper: no struct").clone())
    }
...
fn result_map<T, G, F>(func: F, inputs: &[T], context: &mut Context) -> Result<Vec<G>, UntyperError> where F: Fn(&T, &mut Context) -> Result<G, UntyperError> {
...
fn untype_type(ty: &src::Type, context: &mut Context) -> Result<dst::Type, UntyperError> {
    Ok(match *ty {
        ...
        src::Type::Struct(ref identifier) => dst::Type::Struct(try!(context.get_struct_name(identifier))),
        src::Type::Pointer(ref address_space, ref inner) => dst::Type::Pointer(address_space.clone(), Box::new(try!(untype_type(inner, context)))),
        src::Type::Array(ref inner, dim) => dst::Type::Array(Box::new(try!(untype_type(inner, context))), dim),
...
fn untype_constructor(cons: &src::Constructor, context: &mut Context) -> Result<dst::Constructor, UntyperError> {
    Ok(match *cons {
...
fn untype_intrinsic(instrinic: &src::Intrinsic, context: &mut Context) -> Result<dst::Intrinsic, UntyperError> {
    Ok(match *instrinic {
        src::Intrinsic::GetGlobalId(ref expr) => dst::Intrinsic::GetGlobalId(Box::new(try!(untype_expression(expr, context)))),
    })
}

fn untype_expression(expression: &src::Expression, context: &mut Context) -> Result<dst::Expression, UntyperError> {
    Ok(match *expression {
        src::Expression::Literal(ref literal) => dst::Expression::Literal(literal.clone()),
        src::Expression::Constructor(ref cons) => dst::Expression::Constructor(try!(untype_constructor(cons, context))),
        src::Expression::Local(ref id) => dst::Expression::Variable(try!(context.get_local_name(id))),
        src::Expression::Global(ref id) => dst::Expression::Variable(try!(context.get_global_name(id))),
        ...
        src::Expression::Member(ref expr, ref name) => dst::Expression::Member(Box::new(try!(untype_expression(expr, context))), name.clone()),
        src::Expression::Deref(ref expr) => dst::Expression::Deref(Box::new(try!(untype_expression(expr, context)))),
        src::Expression::MemberDeref(ref expr, ref name) => dst::Expression::MemberDeref(Box::new(try!(untype_expression(expr, context))), name.clone()),
        src::Expression::AddressOf(ref expr) => dst::Expression::AddressOf(Box::new(try!(untype_expression(expr, context)))),
        ...
        src::Expression::Cast(ref ty, ref expr) => dst::Expression::Cast(try!(untype_type(ty, context)), Box::new(try!(untype_expression(expr, context)))),
        src::Expression::Intrinsic(ref intrinsic) => dst::Expression::Intrinsic(try!(untype_intrinsic(intrinsic, context))),
...
fn untype_vardef(vd: &src::VarDef, context: &mut Context) -> Result<dst::VarDef, UntyperError> {
    Ok(dst::VarDef {
        ...
        assignment: match vd.assignment { None => None, Some(ref expr) => Some(try!(untype_expression(expr, context))) },
    })
}
...
fn untype_statement(statement: &src::Statement, context: &mut Context) -> Result<dst::Statement, UntyperError> {
    Ok(match *statement {
        src::Statement::Empty => dst::Statement::Empty,
        src::Statement::Expression(ref expr) => dst::Statement::Expression(try!(untype_expression(expr, context))),
        src::Statement::Var(ref vd) => dst::Statement::Var(try!(untype_vardef(vd, context))),
        src::Statement::Block(ref block) => dst::Statement::Block(try!(result_map(untype_statement, block, context))),
        ...
        src::Statement::For(ref init, ref cond, ref update, ref statement) => {
            dst::Statement::For(
                try!(untype_init_expression(init, context)),
                try!(untype_expression(cond, context)),
        ...
        src::Statement::Return(ref expr) => dst::Statement::Return(try!(untype_expression(expr, context))),
    })
}

fn untype_struct_member(member: &src::StructMember, context: &mut Context) -> Result<dst::StructMember, UntyperError> {
    Ok(dst::StructMember {
...
fn untype_function_param(param: &src::FunctionParam, context: &mut Context) -> Result<dst::FunctionParam, UntyperError> {
...
fn untype_kernel_param(param: &src::KernelParam, context: &mut Context) -> Result<dst::KernelParam, UntyperError> {
    Ok(dst::KernelParam {
        name: try!(context.get_local_name(&param.id)),
        typename: try!(untype_type(&param.typename, context)),
    })
...
fn untype_root_definition(root: &src::RootDefinition, context: &mut Context) -> Result<dst::RootDefinition, UntyperError> {
    Ok(match *root {
        src::RootDefinition::GlobalVariable(ref gv) => {
            dst::RootDefinition::GlobalVariable(dst::GlobalVariable {
                ...
                init: match gv.init { None => None, Some(ref expr) => Some(try!(untype_expression(expr, context))) },
            })
        }
...
pub fn untype_module(module: &src::Module) -> Result<dst::Module, UntyperError> {
    ...
    let mut defs = try!(result_map(untype_root_definition, &module.root_definitions, &mut context));
